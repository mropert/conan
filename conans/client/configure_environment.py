...
class ConfigureEnvironment(object):
    ...
    def _unix_arch_flags(self):
        if self.os == "SunOS":
            return "-m64" if self.arch == "x86_64" else ""
        else:
            return "-m32" if self.arch == "x86" else ""

    def _unix_lib_flags(self):
        lib_flags = []
        if self.libcxx:
            ...
            elif str(self.compiler) == "sunstudio":
    ...
    def _gcc_env(self):
        ...
        archflag = self._unix_arch_flags()
        ...
        all_cpp_flags.extend(self._unix_lib_flags())
    ...
    def command_line_env(self):
        if self.os == "Linux" or self.os == "Macos" or self.os == "FreeBSD":
            ...
            if self.compiler == "gcc" or self.compiler == "clang" or self.compiler == "sunstudio":
                return self._gcc_env()
    ...
    def compile_flags(self):
        if self.compiler == "gcc" or "clang" in str(self.compiler) or self.compiler == "sunstudio":
            flags = []
            flags.extend("-l%s" % lib for lib in self._deps_cpp_info.libs)
            flags.append(self._unix_arch_flags())
            ...
            flags.extend(self._unix_lib_flags())
